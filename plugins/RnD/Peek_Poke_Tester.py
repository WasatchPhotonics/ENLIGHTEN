...
class Peek_Poke_Tester(EnlightenPluginBase):
    ...
    def get_configuration(self):         
        ...
        self.usb_core_device = self.get_usb_device()
...
#############################################
    ...
    def run_peek(self):
        ...
        #usb_core_device = self.get_usb_device()
        ...
        data = self.get_cmd(dev=self.usb_core_device, cmd=0x91, value=address, length=length)
        if data is None:
...
#############################################
    ...
    def run_poke(self):        
        ...
        poke_address = int(self.get_widget_from_name("Address").text().lower().removeprefix("0x"), 16)
        ...
        data = self.send_cmd(dev = self.usb_core_device, cmd = 0x90, value = poke_address, index = len(poke_value), buf = buf)
