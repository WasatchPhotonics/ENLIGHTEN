...
Classes related to ENLIGHTEN plug-ins. Note that [plugins](#plugins) is a different namespace (below).
...
This namespace is technically rooted as a sibling of enlighten.*, in the plugins/
directory.
...
That was so that plugin authors always have access to the _source code_ of their
parent base class, and key objects they were expected to parse or generate. Now
that ENLIGHTEN itself is open-source, this makes less sense, and it would be 
valid to consider moving the plugins.* classes under enlighten.Plugins (physically
and lexically).
