...
class Multispec(object):
    ...
    def __init__(self, button_lock, check_autocolor, check_hide_others,
        colors, combo_spectrometer, desired_serial, frame_widget, graph,
        gui, layout_colors, model_info, reinit_callback, stylesheets,
        eject_button, controller_disconnect, lockable_widgets):
        ...
        self.spec_in_reset = defaultdict(int)
        ...
        self.check_hide_others.stateChanged.connect(self.
        ...
        self.combo_spectrometer.installEventFilter(ScrollStealFilter(self.
            combo_spectrometer))
    ...
    def reject_undesireable(self, device):
        ...
        if self.desired_serial == device.settings.eeprom.serial_number:
            return False
        log.info('rejecting spectrometer (%s != %s)', self.desired_serial,
    ...
    def clear_reset(self, device_id):
        try:
            ...
            log.error(
                "tried to pop device id {device_id} but couldn't find in reset dict"
                )
    ...
    def is_gave_up(self, device_id):
        return device_id in self.in_process and self.in_process[device_id
            ] is None
    ...
    def set_disconnecting(self, device_id, status):
        """
    ...
    def is_disconnecting(self, device_id):
    ...
    def check_ejected_unplugged(self, connected_devices):
        """
        ...
        self.ejected = {dev for dev in self.ejected if dev not in
            physically_unplugged}

    def check_spec_user_ejected(self, device_id):
    ...
    def unhide(self, spec):
        ...
        spec.curve = self.graph.add_curve(pen=self.make_pen(spec), name=
            spec.label, spec=spec)

    def set_state(self, field, value):
        if self.locked:
            for spec in self.get_spectrometers():
                log.debug('%s: settings.state.%s -> %s', spec.label, field,
                    value)
        ...
        try:
            ...
            for device_id in sorted(self.spectrometers, key=str):
                ...
                if spec is not None:
                    ...
                    log.critical(
                        'get_spectrometers: None spectrometer for device_id %s'
                        , device_id)
            return specs
        except Exception as e:
            log.error(f'{e}')
    ...
    def get_clones(self, device_id):
        ...
        if spec is None:
            log.error("Multispec.get_clones: can't find spectrometer %s",
                device_id)
            return clones
        for did in sorted(self.spectrometers, key=str):
            ...
            if other is None or did == device_id:
                continue
            if (other.settings.eeprom.serial_number == spec.settings.eeprom
                .serial_number):
                clones.append(spec)
        return clones
    ...
    def current_spectrometer(self):
        ...
        log.error(
            f"Multispec.current_spectrometer: can't find self.device_id {self.device_id} in spectrometers {self.spectrometers}"
            )
        return None
    ...
    def add(self, device):
        ...
        log.debug('Multispec.add: instantiating Spectrometer for device_id %s',
            device_id)
        ...
        spec.curve = self.graph.add_curve(pen=pen, name=spec.label, spec=spec)
        ...
        if spec.settings.eeprom.has_horizontal_roi():
            ...
            spec.roi_region_left = pyqtgraph.LinearRegionItem((0, roi.start
                ), pen=region_color, brush=region_color, movable=False)
            spec.roi_region_right = pyqtgraph.LinearRegionItem((roi.end,
                spec.settings.eeprom.active_pixels_horizontal), pen=
                region_color, brush=region_color, movable=False)
    ...
    def get_combo_index(self, spec):
        ...
        for index in range(self.combo_spectrometer.count()):
            this_label = self.combo_spectrometer.itemText(index).replace('\n',
                ' ', 1)
            log.debug(
                f'COMBO IDX checking {label} matches {index} {this_label}')
            if label == this_label:
                return index
    ...
    def remove(self, spec=None, silent=False):
        if spec is None:
            spec = self.spectrometers[self.device_id]
        if spec.device_id not in self.spectrometers.keys():
            log.debug(
                f'Failed to find {spec} in {self.spectrometers.keys()}, returning'
                )
            return False
        if spec is None:
            return False
        ...
        if index < 0:
            try:
                ...
                log.error(
                    f'Multispec.remove: failed to delete from spectrometers with error {e}'
                    )
                return False
    ...
    def get_combo_device_id(self):
        ...
        for device_id in sorted(self.spectrometers, key=str):
            if serial == self.spectrometers[device_id
                ].settings.eeprom.serial_number:
                return device_id
        log.error("get_combo_device_id: can't find serial %s, model %s",
            serial, model)
        return None
    ...
    def combo_callback(self):
        ...
        if device_id is None:
            log.debug(
                "combo_callback: can't determine device_id (setting None)")
            self.device_id = None
            return
        spec = self.spectrometers[device_id]
        if spec is None:
            log.error('combo_callback: selected non-existent spectrometer: %s',
                device_id)
            return
        if device_id == self.device_id:
            log.info(self.spectrometers)
            log.debug(
                'combo_callback[%s]: re-selected current spectrometer %s (no-op)'
                , device_id, self.device_id)
            return
        log.info(
            f'combo_callback[{device_id}]: switching default device (now {device_id})'
            )
        self.device_id = device_id
    ...
    def make_pen(self, spec):
        ...
        if self.is_autocolor():
            ...
            color = spec.assigned_color
        try:
            pen = self.gui.make_pen(widget='scope', selected=self.
                is_selected(spec.device_id), color=color)
            return pen
        except:
    ...
    def choose_color(self, spec):
        if self.is_autocolor() and spec.wp_model_info is not None:
            ...
            if model_color in self.seen_model_colors.keys(
                ) and self.seen_model_colors.get(model_color, None) != spec:
                if spec.assigned_color is not None:
                    color = spec.assigned_color
                elif spec.settings.eeprom.serial_number in self.serial_colors:
                    color = self.serial_colors[spec.settings.eeprom.
                        serial_number]
                else:
        ...
        if spec.assigned_color is None:
            log.debug('spectrometer %s permanently assigned color %s', spec
                .label, color)
